...
function wp_login_form( $args = array() ) {
	...
	$form = '<h3>'.__("[:en]Login[:ru]Авторизация[:ro]Logare[:]").'</h3>
		<form name="' . $args['form_id'] . '" id="' . $args['form_id'] . '" action="' . esc_url( site_url( 'wp-login.php', 'login_post' ) ) . '" method="post">
			' . $login_form_top . '
			<p class="login-username">
				<!-- <label for="' . esc_attr( $args['id_username'] ) . '">' . esc_html( $args['label_username'] ) . '</label> -->
				<input type="text" name="log" id="' . esc_attr( $args['id_username'] ) . '" class="input registration" placeholder="E-mail" value="' . esc_attr( $args['value_username'] ) . '" size="20" />
			</p>
			<p class="login-password">
				<!-- <label for="' . esc_attr( $args['id_password'] ) . '">' . esc_html( $args['label_password'] ) . '</label> -->
				<input type="password" name="pwd" id="' . esc_attr( $args['id_password'] ) . '" class="input registration" placeholder="' . __('[:en]Password[:ru]Пароль[:ro]Parolă[:]') . '" value="" size="20" />
			</p>
			' . $login_form_middle . '
			' . ( $args['remember'] ? '<!-- <p class="login-remember"><label><input name="rememberme" type="checkbox" id="' . esc_attr( $args['id_remember'] ) . '" value="forever"' . ( $args['value_remember'] ? ' checked="checked"' : '' ) . ' /> ' . esc_html( $args['label_remember'] ) . '</label></p> -->' : '' ) . '
			<p class="login-submit">
				<input type="submit" name="wp-submit" id="' . esc_attr( $args['id_submit'] ) . '" class="button button-primary registration" value="' . esc_attr( $args['label_log_in'] ) . '" />
