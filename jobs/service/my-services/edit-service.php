	...
	if($_SERVER['REQUEST_METHOD'] === 'POST') {
		...
		if(!$_FILES['serv-photo']['error']) {		
			$name = $_FILES['serv-photo']['name'];	
				...
				move_uploaded_file($_FILES['serv-photo']['tmp_name'], "../../../serv_data/users/$wp_username/$id.jpg");		
		}		
		$qual = (isset($_POST['serv-qual']) && $_POST['serv-qual'] == 0) ? 1 : 0;
		$unqual = (isset($_POST['serv-unqual']) && $_POST['serv-unqual'] == 0) ? 1 : 0;
		...
		$data = array(
			...
			'has_photo' => file_exists("../../../serv_data/users/$wp_username/$id.jpg")		
		);		
		$data_pattern = array('%d', '%s', '%s', '%d', '%d', '%d', '%s', '%d', '%s', '%d');					
		$where = array('id' => $_POST['serv-id']);
		...
		if(!$result) {
			die(json_encode(array("error" => "no serv under that id"), JSON_UNESCAPED_UNICODE));
		}
		...
		$data = array(
			...
			'msg' => $result[0]['msg'],
			'has_photo' => file_exists("../../../serv_data/users/".$result[0]['wp_username']."/".$id.".jpg"),
			'path' => "../../../serv_data/users/".$result[0]['wp_username']."/".$id.".jpg"
		);
