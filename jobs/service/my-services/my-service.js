$(document).ready(function() {
	...
	$.ajax({
		type: "GET",
		url: "/jobs/service/my-services/my-service.php",
		async: true,
		cache: false,
		contentType: false,
		processData: false,
				...
				if(obj.data === null) {
					...
					for(var i = 0; i < obj.data.length; i++) {
						...
						if(obj.data[i].has_photo) {
							src = "https://urbanconsulting.md/serv_data/users/"+obj.data[i].wp_username+"/"+obj.data[i].id+".jpg?"+(new Date()).getTime();
						}
						var state;
						...
						var htmlForm = '\
							<li class="notification bs '+state+'">\
								...
								<div class="col-9 description">\
									<div class="align"><a href="https://urbanconsulting.md/job/services/edit?id='+obj.data[i].id+'">'+obj.data[i].name+'</a></div><br>\
								</div>\
...
function test(id, wp_username) {
	$.ajax({
		type: "POST",
		url: "/jobs/service/my-services/delete-service.php",
		async: true,
		cache: false,
		processData: false,
