<?php
	...
	$state = $accept ? 1 : 2;
	$txt = "Здравствуйте, ".$_POST['fio'].". Ваша услуга была ".($accept ? "одобрена" : "отклонена").". :\n ".$_POST['comment']."\n https://urbanconsulting.md/ru/job/services/";
	
	$data = array(
		'state' => $state,
		'msg' => strlen($_POST["comment"]) ? $_POST["comment"] : "Услуга заполнена неправильно.",
	...
	$subject = "Рассмотрение заявки услуги";
	$headers = "From: office@urbanconsulting.md" . "\r\n" .
	"CC: office@urbanconsulting.md";

	mail($to, $subject, $txt, $headers);
	header('Location: https://urbanconsulting.md/wp-admin/admin.php?page=sub-page2');
?>