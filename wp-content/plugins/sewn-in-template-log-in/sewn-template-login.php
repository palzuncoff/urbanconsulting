	...
	public function settings()
	{
		...
		$this->settings    = array(
			...
			'strings' => array(
				...
				'lost_password_submit'       => __( "[:en]Get New Password[:ru]Выслать пароль[:ro]Expediaza parola[:]", $this->plugin_name ),
				'recover_link_text'          => __( "[:en]Lost password?[:ru]Забыли пароль?[:ro]Ai uitat parola?[:]", $this->plugin_name ),
				...
				'notification_loggedout'     => __( "[:en]You are now logged out.[:ru]Отныне вы не авторизованы.[:ro]Esti pokinutiy.[:]", $this->plugin_name ),
				'notification_recovered'     => __( "[:en]Check your e-mail for the confirmation link.[:ru]Проверьте свой электронный ящик на наличие ссылки для подтверждения.
[:ro]Verifica-ti e-mail-ul pentru link-ul de confirmare.[:]", $this->plugin_name ),
				'notification_password'      => __( "[:en]Please enter your username or email. You will receive a link to create a new password via email.[:ru]Введите ваш логин или e-mail адрес указанный вами при регистрации, на него будет выслан новый пароль.[:ro]Introduceti login-ul dvs. sau adresa de email pe care le-ati indicat la inregistrare, acolo o sa va fie expediata parola noua.[:]", $this->plugin_name ),
				'notification_passworderror' => __( "[:en]<strong>ERROR</strong>: Enter a username or e-mail address.[:ru]<strong>ОШИБКА</strong>: Введите ваш логин или e-mail адрес.[:ro]<strong>EROARE</strong>: Introduceti login-ul dvs. sau adresa de email.[:]", $this->plugin_name ),
			),
		);
...
?>
		<h3><?php _e('[:en]Password recovery[:ru]Востановление пароля[:ro]Restabilirea parolei[:]'); ?></h3>
					...
					<div class="loginform-container">
						<form name="loginform" id="loginform" action="<?php echo site_url('wp-login.php?action=lostpassword', 'login_post'); ?>" method="post">
							<p class="login-username">
								...
								<input type="text" name="user_login" placeholder="<?php _e('[:en]Username or login[:ru]Логин или E-mail[:ro]Login sau E-mail[:]'); ?>" id="user_login" class="input registration" value="" size="20" />
							</p>
							<p class="password-submit">
								<?php do_action( 'login_form', 'resetpass' ); ?>
								<input type="submit" name="wp-submit" id="wp-submit" class="button button-primary registration" value="<?php echo $this->settings['strings']['lost_password_submit']; ?>">
