...
//								array( &$taxes, $price, $rates, $price_includes_tax, $suppress_rounding ) );
	...
	private static function get_matched_tax_rates( $country, $state, $postcode, $city, $tax_class, $valid_postcodes ) {
		...
		global $wpdb;
		$valid_postcodes = implode( "','", $valid_postcodes );
		if($valid_postcodes!= ''){
			$valid_postcodes = "'".$valid_postcodes."'";
		}

		$valid_postcodes = explode(",", $valid_postcodes);
		//$valid_postcodes = array_map( array( esc_sql($f)), $valid_postcodes );

		$match_country   = ($country != '')? "'".strtoupper(  $country  )."'": "''";
		$match_state     = ($state != '')? "'".strtoupper(  $state )."'": "''";
		$match_tax_class = ($tax_class != '')? "'".$tax_class."'": "''";
		$match_city      = ($city != '')? "'".strtoupper( $city )."'": "''";
		...
		$found_rates = $wpdb->get_results($query);
...
//								array( &$matched_tax_rates, $country, $state, $postcode, $city, $tax_class ) );
	...
	public static function get_tax_location( $tax_class = '' ) {
		...
		if ( $session->has('customer_billing_country') ) {
			$country_id = $session->get('customer_billing_country', $config->get('country','') );
			$zone_id 	= $session->get('customer_billing_state', $config->get('region',''));
			$zip 		= $session->get('customer_billing_zip', $config->get('zip',''));
			$city 		= $session->get('customer_billing_city', $config->get('city',''));
